...
def run_shrinkage_grid_experiment():
    config = BacktestConfig(
        ...
        end_date='2025-12-30',
        window_months=60,
        result_dir='result_shrinkage_grid'
    )

    print("\n" + "=" * 50)
    print("STARTING SHRINKAGE GRID SEARCH EXPERIMENT")
    print("=" * 50)
    ...
    res_sample = run_single_strategy_backtest(
        ...
        strategy_name='Sample Cov', build_sigma_fn=sample_cov_builder, omega_method='advanced', kappa=FIXED_KAPPA
    )
    strategy_results[res_sample["name"]] = res_sample

    shrinkage_grid = [0.25, 0.50, 0.75, 1.00]

    def make_shrinkage_builder(strength):
        def shrink_cov_builder(daily_window: pd.DataFrame) -> np.ndarray:
            return np.asarray(shrinkage_covariance(daily_window, shrinkage_strength=strength)) * 21

        return shrink_cov_builder

    for s in shrinkage_grid:
        name = f"Shrinkage (s={s:.2f})"
        res_shrink = run_single_strategy_backtest(
            config=config, monthly_rets=monthly_rets, daily_rets=daily_rets, macro_df=macro_df,
            strategy_name=name, build_sigma_fn=make_shrinkage_builder(s), omega_method='advanced', kappa=FIXED_KAPPA
        )
    ...
    print("\n" + "=" * 90)
    print("SHRINKAGE STRENGTH GRID EVALUATION MATRIX")
    print("=" * 90)
...
# =============================================================================
# EXPERIMENT 2: FIXED KAPPA (SAMPLE VS L2 VS SHRINKAGE)
# =============================================================================
...
def run_fixed_kappa_covariance_experiment():
    ...
    print("\n" + "=" * 50)
    print("STARTING COVARIANCE COMPARISON EXPERIMENT")
    print("=" * 50)
    ...
    res_sample = run_single_strategy_backtest(
        config=config, monthly_rets=monthly_rets, daily_rets=daily_rets, macro_df=macro_df,
        strategy_name='Fixed Kappa (Sample Cov)', build_sigma_fn=sample_cov_builder, omega_method='advanced',
        kappa=FIXED_KAPPA
    )
    strategy_results[res_sample["name"]] = res_sample
    ...
    res_l2 = run_single_strategy_backtest(
        config=config, monthly_rets=monthly_rets, daily_rets=daily_rets, macro_df=macro_df,
        strategy_name='Fixed Kappa (L2 Reg Cov)', build_sigma_fn=l2_cov_builder, omega_method='advanced',
        kappa=FIXED_KAPPA
    )
    ...
    res_shrink = run_single_strategy_backtest(
        config=config, monthly_rets=monthly_rets, daily_rets=daily_rets, macro_df=macro_df,
        strategy_name='Fixed Kappa (Shrinkage Cov)', build_sigma_fn=shrink_cov_builder, omega_method='advanced',
        kappa=FIXED_KAPPA
    )
    strategy_results[res_shrink["name"]] = res_shrink
